...
- `--nlp-table=TABLE`: only build this table from the workflow, instead of all of them. Pass it
  more than once to build a subset of tables (e.g. `--nlp-table=age --nlp-table=race`). This lets
