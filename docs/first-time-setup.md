...
You can use `--target` to specify a specific study to be run. You can use `--study-dir`
with most arguments to target a directory where you are working on studies/working with
studies that aren't available to install with `pip`.
...
| --s3_staging_dir | CUMULUS_S3_STAGING_DIR |
