...
class PsmPy:
    ...
    def __init__(self, data, treatment, indx, exclude=[], target="outcome", seed=42):
        ...
        self.data[treatment] = self.data[treatment].astype(int)  # need binary 0, 1
        ...
        if not any(pandas.api.types.is_numeric_dtype(self.dataIDindx[xvar]) for xvar in self.xvars):
            raise TypeError("Only numeric dtypes allowed")
    ...
    def cohenD(self, df, treatment, metricName):
        """
    ...
    def logistic_ps(self, balance=True):
        ...
        if balance:
            ...
            if majority_len_diff != 0:
                ...
                if len_major_leftover <= 20:
                    ...
                    # sample from the ones already folded over above
                    # to get major class of 20 for last fold:
                    ...
                    majority_leftover_all = pandas.concat([majority_leftover1, majority_leftover2])
                    # sample a macthing 20 from the minor class
                    minority_sample = minority.sample(n=20, random_state=self.seed)
                    joint_df = pandas.concat([majority_leftover_all, minority_sample])
                    treatment = joint_df[self.treatment]
                    df_cleaned = joint_df.drop([self.treatment], axis=1)
                    logistic = LogisticRegression(solver="liblinear", random_state=self.seed)
                    logistic.fit(df_cleaned, treatment)
                    ...
                    minority_sample = minority.sample(n=majority_len_diff, random_state=self.seed)
                    joint_df = pandas.concat([majority_extra, minority_sample])
                    treatment = joint_df[self.treatment]
                    df_cleaned = joint_df.drop([self.treatment], axis=1)
                    logistic = LogisticRegression(solver="liblinear", random_state=self.seed)
                    logistic.fit(df_cleaned, treatment)
                ...
                pass
            predicted_data_repeated = pandas.concat(appended_data)
            ...
            repeated_data = predicted_data_repeated_reset[
                predicted_data_repeated_reset.duplicated(subset=self.indx, keep=False)
            ]
            ...
            filtered_repeated = pandas.concat(mean_repeated)

            # remove all duplicated minority class from folded df to be rejoined
            # with the fixed values
            not_repeated_predicted = predicted_data_repeated_reset.drop_duplicates(
                subset=self.indx, keep=False
            )
            predicted_data_ps = pandas.concat(
                [filtered_repeated, not_repeated_predicted]
            ).reset_index(drop=True)

            # merge with treatment df
            treatment_dfonly = self.dataIDindx[[self.treatment]].reset_index()
            self.predicted_data = pandas.merge(
        ...
        # If user does not wish cases to be balanced
            ...
            joint_df = pandas.concat([majority, minority])
            treatment = joint_df[self.treatment]
            df_cleaned = joint_df.drop([self.treatment], axis=1)
            logistic = LogisticRegression(solver="liblinear", random_state=self.seed)
            logistic.fit(df_cleaned, treatment)
    ...
    def knn_matched(
        self, matcher="propensity_logit", replacement=False, caliper=None, drop_unmatched=True
    ):
        """
        ...
        # caliper = numpy.std(predicted_data_control[matcher]) * caliper_multip
        # else:
        # pass
        ...
        if caliper is not None:
            ...
            for dist, row in zip(distances[:, :], indexes[:, :]):
                ...
                dist = numpy.ndarray.tolist(dist)
                row = numpy.ndarray.tolist(row)
                # finds indices to include from distances based on caliper:
                dist_indices = [
                    idx for idx, element in enumerate(dist) if condition_caliper(element, caliper)
                ]
                ...
                if not replacement:
                    if len(elements_to_remove) > 0:
                        ...
                        try:
                            ...
                            # append None if nothing within caliper range that isn't a duplicate
        ...
        if drop_unmatched:
            if len(indexes[:, 1]) == len(list(filter(None, indices_for_match))):
                ...
                warnings.warn(
                    "Some values do not have a match. These are dropped for purposes of "
                    "establishing a matched dataframe, and subsequent calculations and plots "
                    "(effect size). If you do not wish this to be the case please set "
            ...
            indices_to_take_from_minor_class = list(matched_ids_nona.index.values)
            minor_pred_rstindx = minor_pred_rstindx.take(indices_to_take_from_minor_class)
            self.df_matched = pandas.concat(
            ...
            if len(indexes[:, 1]) == len(list(filter(None, indices_for_match))):
                ...
                warnings.warn(
                    "Some values do not have a match. These are not dropped for purposes of "
                    "subsequent calculation and plots. If you do not wish this to be the case "
                    "please set drop_unmatched=True"
                )
    ...
    def knn_matched_12n(self, matcher="propensity_logit", how_many=1):
        ...
        ID_match = []
        # indexes for match needs to keep all those tagged from all loops so must be instantiated
        # outside the loop
        for many in range(how_many):
            ...
            # remove all used indexes from this round of KNN and prepare for next with the
            # last set removed:
            # create df for exclusion on those that have matched
            major_matched_loop = major_pred_rstindx.take(indices_in_loop)
            # pull patient indexes (self.indx) from dataframe as we go (since array values
            # will repeat):
            ...
            major_pred_rstindx = pandas.merge(
                major_pred_rstindx,
                major_matched_loop[[self.indx]],
                on=self.indx,
                how="outer",
                indicator=True,
        ...
        df_2_extract_major_class = pandas.DataFrame(ID_match, columns=[self.indx])
        df_4_user = pandas.merge(
            major_pred_rstindx_og, df_2_extract_major_class, on=self.indx, how="right"
        )

        self.df_matched = pandas.concat([minor_pred_rstindx, df_4_user], axis=0, ignore_index=True)

        # build df of matched ids passed back to user:
        # create array for easy viewing of the 'n' levels of macthing performed
        ID_match_array = numpy.asarray(ID_match).reshape((len(minor_pred_rstindx), how_many))
        major_list_cols = []
        for many in range(how_many):
            col_name = "largerclass_" + str(many) + "group"
            major_list_cols.append(col_name)
        # convert matched ids from major class to df
        major_class_matched_ids_df = pandas.DataFrame(ID_match_array, columns=major_list_cols)
    ...
    def get_histogram(
        ...
        """
        get_histogram - Returns human-labeled knn matched data.
        Parameters
        ----------
        names  : list
           List of 2 groups
        ...
        pandas dataframe
        """
        df_matched = self.df_matched.copy()
        df_matched[self.treatment] = df_matched[self.treatment].replace({0: names[0], 1: names[1]})
        return df_matched
    ...
    def get_effect_size(
        ...
        """
        get_effect_size -- Returns effect size on each variable before and after matching
        Parameters
        ----------
        Returns
        pandas dataframe
        ...
        for cl in self.xvars:
            data.append([cl, "before", self.cohenD(df_preds_b4_float, self.treatment, cl)])
            data.append([cl, "after", self.cohenD(df_preds_after_float, self.treatment, cl)])
        self.effect_size = pandas.DataFrame(data, columns=["Variable", "matching", "Effect Size"])
