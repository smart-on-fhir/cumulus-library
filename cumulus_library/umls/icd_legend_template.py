...
def execute_templates(cursor, schema: str, verbose: bool, partition_size: int = 2000):
    ...
    query_count = 2  # accounts for static CTAS/legend queries
    ...
    if verbose:
        ...
        build_icd_legend(cursor, verbose, progress, task)
    else:
        with Progress() as progress:
            task = progress.add_task(
                f"Uploading {table_name} data...", total=query_count
            )
            build_umls_icd(
                cursor,
                schema,
...
def build_umls_icd(
    cursor, schema, verbose, partition_size, table_name, path, icd_files, progress, task
):
    ...
    for filename in icd_files:
        with open(f"{path}/{filename}.bsv", "r") as file:
            ...
            if not created:
                row = clean_row(next(reader), filename)
...
def build_icd_legend(cursor, verbose, progress, task):
    legend_query = """create or replace view icd_legend AS 
    select code, display, code_display, system from (
        SELECT code, str as display, CONCAT(code,' ',str) as code_display, SAB as system, row_number()
        OVER (PARTITION BY SAB, CODE
        ORDER BY length(str) asc) AS PREF
        FROM umls_icd
    )
    where PREF = 1;"""
    cursor.execute(legend_query)
    query_console_output(verbose, legend_query, progress, task)
