...
def add_config_file_argument(parser: argparse.ArgumentParser) -> None:
    """Adds a --file arg to a subparser for reading in arg configs

    NOTE: always include this last, so that the cli args can override it if
    explicitly set
    """
    parser.add_argument(
        "--file",
...
AWS Athena, the following order of preference is used to select credentials:
  ...
  - Normal boto profile order (AWS env vars, ~/.aws/credentials, ~/.aws/config)""",
    ...
    add_config_file_argument(clean)
    ...
    add_config_file_argument(export)
    # Database import
    ...
    add_config_file_argument(importer)
    ...
    add_config_file_argument(upload)
    ...
    add_config_file_argument(markdown)
    ...
    add_config_file_argument(version)
